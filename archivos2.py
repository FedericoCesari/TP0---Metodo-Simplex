...
def preparar_simplex(filename):
    ...
    # Obtener la función objetivo (1ra fila), que es la primera línea del archivo
    ...
    num_variables = len(funcion_objetivo)        # Num de variables originales
...
def verificacion(filename):
    ...
    try:
        restricciones = []
        for line in lines:
            # Convertir la línea en una lista de números float
            row = list(map(float, line.strip().split()))
            restricciones.append(row)
            return True
    except ValueError:
